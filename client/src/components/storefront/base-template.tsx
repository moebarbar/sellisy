...
function BaseTemplateInner({ store, products, bundles, theme, testimonials = [], faqs = [], reviews = [], subscriberCount = 0 }: BaseTemplateProps) {
  ...
  const [paymentModalProduct, setPaymentModalProduct] = useState<StorefrontProduct | null>(null);
  const [checkoutLoading, setCheckoutLoading] = useState(false);

  const storeHasStripe = (store as any).stripeSecretKey === "***configured***";
  const storeHasPayPal = !!(store as any).paypalClientId && (store as any).paypalClientSecret === "***configured***";
  ...
  const processCheckout = async (product: StorefrontProduct, paymentMethod?: "stripe" | "paypal") => {
    ...
    try {
      const body: Record<string, string> = { storeId: store.id, productId: product.id };
  ...
  return (
    <div style={{ background: c.bg, color: c.text }} className="min-h-screen relative overflow-hidden">
                    ...
                    <div className="relative overflow-hidden" style={{ borderRadius: `${theme.layout.cardBorderRadius} ${theme.layout.cardBorderRadius} 0 0` }}>
                      <a href={`${basePath}/product/${product.slug || product.id}`} data-testid={`link-product-img-${product.id}`}>
                        {product.thumbnailUrl ? (
                          <ProtectedImage protected={!store.allowImageDownload} src={product.thumbnailUrl} alt={product.title} className={`w-full ${theme.layout.imageAspect || "aspect-square"} object-cover transition-transform duration-700 group-hover:scale-105`} loading="lazy" data-testid={`img-product-${product.id}`} />
                        ) : (
                          <StorefrontProductPlaceholder productType={product.productType} accentColor={store.accentColor || undefined} title={product.title} className={theme.layout.imageAspect || "aspect-square"} />
                        )}
                      </a>
      ...
      {paymentModalProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" data-testid="modal-payment-method">
          <div className="absolute inset-0 bg-black/50" onClick={() => setPaymentModalProduct(null)} />
          <div className="relative z-10 w-full max-w-md mx-4 rounded-2xl p-6 shadow-2xl" style={{ background: c.card, border: `1px solid ${c.cardBorder}` }}>
            <button
            ...
            <h3 className="text-lg font-semibold mb-1" style={{ color: c.text, fontFamily: theme.typography.headingFamily }}>
              Choose payment method
            </h3>
            <p className="text-sm mb-5" style={{ color: c.textSecondary, fontFamily: theme.typography.bodyFamily }}>
              How would you like to pay for {paymentModalProduct.title}?
            </p>
            <div className="flex flex-col gap-3">
              <button
                onClick={() => processCheckout(paymentModalProduct, "stripe")}
                disabled={checkoutLoading}
                className="flex items-center gap-3 w-full rounded-xl px-4 py-3.5 text-left font-medium transition-all hover:scale-[1.02]"
                style={{ background: c.bg, border: `1px solid ${c.cardBorder}`, color: c.text, fontFamily: theme.typography.bodyFamily }}
                data-testid="button-pay-stripe"
              >
                <div className="flex items-center justify-center h-10 w-10 rounded-lg" style={{ background: "#635bff" }}>
                  <svg viewBox="0 0 24 24" fill="white" className="h-5 w-5"><path d="M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.992 3.757 7.218c0 4.039 2.467 5.76 6.476 7.219 2.585.92 3.445 1.574 3.445 2.583 0 .98-.84 1.545-2.354 1.545-1.875 0-4.965-.921-6.99-2.109l-.9 5.555C5.175 22.99 8.385 24 11.714 24c2.641 0 4.843-.624 6.328-1.813 1.664-1.305 2.525-3.236 2.525-5.732 0-4.128-2.524-5.851-6.591-7.305z"/></svg>
                </div>
                <div>
                  <span className="text-sm font-semibold">Pay with Card</span>
                  <span className="block text-xs" style={{ color: c.textSecondary }}>Powered by Stripe</span>
                </div>
              </button>
              <button
                onClick={() => processCheckout(paymentModalProduct, "paypal")}
                disabled={checkoutLoading}
                className="flex items-center gap-3 w-full rounded-xl px-4 py-3.5 text-left font-medium transition-all hover:scale-[1.02]"
                style={{ background: c.bg, border: `1px solid ${c.cardBorder}`, color: c.text, fontFamily: theme.typography.bodyFamily }}
                data-testid="button-pay-paypal"
              >
                <div className="flex items-center justify-center h-10 w-10 rounded-lg" style={{ background: "#003087" }}>
                  <svg viewBox="0 0 24 24" fill="white" className="h-5 w-5"><path d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74L4.944.901C5.026.382 5.474 0 5.998 0h7.46c2.57 0 4.578.543 5.69 1.81 1.01 1.15 1.304 2.42 1.012 4.287-.023.143-.047.288-.077.437-.983 5.05-4.349 6.797-8.647 6.797h-2.19c-.524 0-.968.382-1.05.9l-1.12 7.106zm14.146-14.42a3.35 3.35 0 0 0-.607-.541c-.013.076-.026.175-.041.254-.93 4.778-4.005 7.201-9.138 7.201h-2.19a.563.563 0 0 0-.556.479l-1.187 7.527h-.506l-.24 1.516a.56.56 0 0 0 .554.647h3.882c.46 0 .85-.334.922-.788.06-.26.76-4.852.816-5.09a.932.932 0 0 1 .923-.788h.58c3.76 0 6.705-1.528 7.565-5.946.36-1.847.174-3.388-.777-4.471z"/></svg>
                </div>
                <div>
                  <span className="text-sm font-semibold">Pay with PayPal</span>
                  <span className="block text-xs" style={{ color: c.textSecondary }}>Redirect to PayPal</span>
                </div>
              </button>
            </div>
            {checkoutLoading && (
              <p className="text-center text-sm mt-4" style={{ color: c.textSecondary }}>Processing...</p>
            )}
          </div>
        </div>
      )}
