...
4. ~~**Basic course product type**~~ ✅ **Shipped** — `course_lessons` + `course_lesson_progress` tables. Owner edits lessons inside the product editor (title, video URL, description, attachment, duration, reorder). Buyer accesses via `/course/:token/:productId` — sidebar lesson list, video player with YouTube/Vimeo URL transformation, mark-complete button, progress bar. Vs-pages refreshed: Podia / Payhip FAQ + matrix rows updated.
5. ~~**PDF watermarking**~~ ✅ **Shipped** — per-store toggle. When enabled, single-file PDF downloads are stamped with buyer email + order ID footer + diagonal "PERSONAL COPY" watermark via `pdf-lib`. 30 MB size cap (passes through unstamped above that). Vs-pages: SendOwl PDF-stamping row flipped from "competitor wins" to "tie".
