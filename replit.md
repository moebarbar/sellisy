...
## Recent Changes
- 2026-02-18: **Visual template selector** — Replaced template dropdown with visual preview cards showing mini-storefront mockups for each template (Neon/Silk). Reusable `TemplateSelector` component used in both Create Store dialog and Store Settings. Scalable TEMPLATES array for adding more templates. Fixed product detail page vertical scroll (overflow-hidden → overflow-x-hidden).
- 2026-02-18: **Marketing Playbook** — Dashboard section with 26 curated marketing strategies across 8 categories (Launch, Email Marketing, Social Media, SEO, Content Marketing, Sales Strategy, Paid Ads, Analytics). Each strategy includes actionable steps, difficulty/impact ratings, and rich markdown content guides. Strategy detail pages at /dashboard/marketing/:strategyId with custom markdown renderer. Per-store progress tracking (Not Started/In Progress/Completed) with overall progress bar. Category and status filters. Tables: marketingStrategies (seeded content with `content` column), storeStrategyProgress (per-store tracking). API: GET /api/marketing/strategies, GET/PATCH /api/marketing/progress/:storeId.
