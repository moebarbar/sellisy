...
*   **Storefronts**: Two primary templates, 'Neon' (dark, bold) and 'Silk' (elegant, minimal), are available as legacy standalone components. A **unified base template system** (`BaseTemplate` + `StorefrontTheme` configs) enables rapid creation of new templates by defining only visual tokens (colors, fonts, layout, effects). Theme configs live in `client/src/components/storefront/themes/`. Stores support announcement bars, social media links, rich footers, category navigation, search/sort, and scroll-reveal animations. Each store has a branded customer portal.
*   **Product Management**:
